...
int main(){
 ...
 Check(keys.size()==32&&original.size()==55,"catalog union");
 for(const auto& preset:openq4::PerformancePresets()){
  ...
  Check(untouched==23&&SettingsValuesEqual(tx.Baseline(),original),"23 outside keys and baseline preserved");
  Check(writes==0,"draft preset never invokes setter");
  StateValues readback;Check(host.Read(readback,error)&&SettingsValuesEqual(original,readback),"all 55 live unchanged");
 ...
 try:
  ...
  if args.mutations:
   ...
   mutations=[
    ...
    ('merged-validation','native','src/ui/application/SettingsTransaction.cpp','if(!ValidateDraft(candidate,error))return false;','if(false)return false;'),
