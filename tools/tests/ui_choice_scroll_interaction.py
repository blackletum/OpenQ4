...
def main():
 ...
 if args.mutations and r is not None and r.returncode==0:
  ...
  # Both invalidation and measurement now guard the popup identity. Target the
  # invalidation method explicitly so the mutation still tests its own contract.
  prefix='bool Interaction::InvalidateChoicePopup(const std::string& id,std::uint64_t expected) {\n    '
  rules=[(name,prefix+old,prefix+new) if name.startswith('invalidate_') else (name,old,new) for name,old,new in rules]
  try:
