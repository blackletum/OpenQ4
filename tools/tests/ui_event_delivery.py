...
int main() {
    ...
    }
    {
        ...
        s.session.PumpApplicationActions();
        // The clipboard boundary now rejects reentrant owner dispatch before
        // the outer pump's depth limit; queued work survives for the next pump.
        assert(applicationActions.size()==1 && gui->pendingActions.size()==1);
        gui->onDispatch={}; s.session.PumpApplicationActions();
        assert(applicationActions.size()==2 && gui->pendingActions.empty());
    }
    {
        Scenario s;
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix='ui-event-delivery-', dir=ROOT / '.tmp') as directory:
        ...
        subprocess.run([compiler, '-std=c++20', '-I', str(ROOT), str(test), str(ROOT / 'src/ui/retained/TextInput.cpp'), '-o', str(binary)], check=True)
