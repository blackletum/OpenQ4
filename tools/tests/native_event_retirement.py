...
def main():
    ...
    if not args.no_mutations:
        cases += [
            ('platform-thread','windows','platform','if (!Sys_EventDispositionBoundThread()) return sysEventTransfer_t::Refused;','if (false) return sysEventTransfer_t::Refused;',2),
            ...
            ('pushed-journal-cleanup-block','posix','loop','if (!Sys_EventDispositionBoundThread()) return sysEventTransfer_t::Refused;','if (!Sys_EventDispositionBoundThread() || com_journal.GetInteger()!=0) return sysEventTransfer_t::Refused;',3),
