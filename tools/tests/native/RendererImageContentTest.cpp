...
static std::vector<byte> Bimage(int sides,bool reverse=false,int levels=4,textureFormat_t format=FMT_RGBA8,unsigned magic=BIMAGE_MAGIC) {
    std::vector<byte> data;PutBig(data,987,8);PutBig(data,magic);
    for(int n:{int(sides==6?TT_CUBIC:TT_2D),int(format),int(CFM_DEFAULT),8,8,levels})PutBig(data,n);
    for(int record=0;record<sides*levels;++record){int index=reverse?sides*levels-record-1:record;int side=index/levels,level=index%levels,size=8>>level;
        for(int n:{level,side,size,size,size*size*4})PutBig(data,n);
        data.insert(data.end(),size*size*4,byte(side*20+level+7));
    }return data;
}
static void PackedCompatibility() {
    // Real packed-image entry point at a nonzero stream offset, with a following
    // chunk that must remain unread. Version 10 remains authored pack content,
    // but is deliberately obsolete as a regenerable generated-image cache.
    for(unsigned version:{9u,10u,unsigned(BIMAGE_VERSION),12u})for(int sides:{1,6})for(int levels:{1,4})for(auto format:{FMT_RGBA8,FMT_SRGBA8}) {
        const unsigned magic=(unsigned('B')|(unsigned('I')<<8)|(unsigned('M')<<16)|(version<<24));
        const auto bytes=Bimage(sides,false,levels,format,magic);
        fs.file.bytes.assign(13,0x65);fs.file.bytes.insert(fs.file.bytes.end(),bytes.begin(),bytes.end());fs.file.bytes.insert(fs.file.bytes.end(),17,0x76);fs.file.cursor=13;
        idBinaryImage image;
        const bool admitted=version==BIMAGE_VERSION||(version==10&&sides==1&&levels==1);
        TEST(image.LoadFromFile(&fs.file,int(bytes.size()))==admitted);
        TEST(fs.file.cursor==13+int(bytes.size()));
        if(admitted){TEST(image.images.size()==size_t(sides*levels));TEST(image.images[0].data[0]==7);TEST(image.fileData.format==format);}
        else TEST(image.images.empty()&&image.loadedFileData==nullptr);
        fs.file.bytes=bytes;fs.file.cursor=0;
        TEST(image.LoadFromFile(&fs.file)==(version==BIMAGE_VERSION));
        TEST(image.LoadFromGeneratedFileUnchecked()==(version==BIMAGE_VERSION?73:FILE_NOT_FOUND_TIMESTAMP));
        TEST(image.LoadFromCompactGeneratedFileUnchecked()==(version==BIMAGE_VERSION?73:FILE_NOT_FOUND_TIMESTAMP));
        imageFileContent_t identity;
        TEST(R_MakeImageFileContent(IFC_OBSERVED_BIMAGE,"generated/packed.bimage",bytes.data(),bytes.size(),identity));
        TEST(image.LoadExactContentFile(identity)==(version==BIMAGE_VERSION));
        if(!admitted)continue;
        for(int mode=0;mode<4;++mode){fs.file.bytes=bytes;fs.file.cursor=0;int count=int(bytes.size());
            if(mode==0){fs.file.bytes.pop_back();--count;}
            if(mode==1){fs.file.bytes.push_back(0);++count;}
            if(mode==2)fs.file.bytes[sizeof(bimageFile_t)+3]=99;
            if(mode==3)++count;
            TEST(!image.LoadFromFile(&fs.file,count));TEST(image.images.empty()&&image.loadedFileData==nullptr);
        }
    }
}
...
int main(){try{PathsAndDigests();Direct();Cached();PackedCompatibility();TEST(fs.open==0&&allocations.empty());std::printf("RendererImageContentTest PASS %d checks\n",checks);return 0;}catch(const std::exception&e){std::fprintf(stderr,"FAIL %s (%d checks)\n",e.what(),checks);return 1;}}
