...
static void R_DoneFreeType(bool preserveAtlasImages = false) {
...
static bool VK_GuiExecutor_FrameIsOpen(){return false;}
static bool VK_GuiExecutor_EndFrameAndPresent(){return true;}
'''
...
struct { bool initialized=false; struct { uint32_t width=640,height=480; } swapchainExtent; } vkCtx;
struct idStr {template<class... T> static void snPrintf(char* out,int size,const char* format,T... args){std::snprintf(out,size,format,args...);}};
static const char* vkStartupDeviceStage="window system";
static bool VK_Device_InjectStartupFailure(int){return false;}
static void VK_Bringup_ReleaseHeldInstance(){}
static bool VK_GuiExecutor_PrepareStartup(){events.emplace_back("resources");return true;}
static void R_RendererMetrics_ShutdownGpuTimers(){}
...
struct Services {
    ...
    void BeginWindowTeardown(){}
    void FinishWindowTeardown(){if(windowAlive)DestroyAttemptWindow();}
...
static bool VK_Device_Init(Services* value) {
    ...
    events.emplace_back("device-init"); deviceAlive=vkCtx.initialized=deviceOkay;
    vkCtx.swapchainExtent={640,480}; return deviceOkay;
}
static void VK_Device_Shutdown() {
    assert(!glConfig.isInitialized);
    if(deviceAlive)events.emplace_back("device-shutdown");deviceAlive=vkCtx.initialized=false;
}
...
static void Reset() {
    ...
    preserved=deviceAlive=windowAlive=false;glConfig={};vkCtx={};pixelWidth=1280;pixelHeight=720;
...
int main() {
    ...
    assert(!VK_InitRenderDevice());Expect({"warning","warning"});
    Reset();prepareOkay=false;
    assert(!VK_InitRenderDevice());Expect({"prepare","warning","warning"});
    Reset();createOkay=false;
    assert(!VK_InitRenderDevice());Expect({"prepare","window-create","warning","warning"});
    Reset();deviceOkay=false;
    assert(!VK_InitRenderDevice());Expect({"prepare","window-create","device-init","window-destroy","warning"});
    for(bool keepWindow:{false,true}) {
        for(bool failWindow:{false,true}) {
            ...
            if(failWindow)Expect({"prepare","window-create","device-init","window-apply","device-shutdown","window-destroy","warning"});
            else Expect({"prepare","window-create","device-init","window-apply","refresh","recreate","device-shutdown","window-destroy","warning"});
            ...
            assert(VK_InitRenderDevice());
            Expect({"prepare","window-create","device-init","window-apply","refresh","recreate","resources","refresh","config","ready","input"});
            assert(deviceAlive && windowAlive && glConfig.isInitialized);
    ...
    assert(VK_InitRenderDevice());
    Expect({"prepare","window-create","device-init","window-apply","refresh","resources","refresh","config","ready","input"});
    Reset();initialSizeOkay=false;assert(!VK_InitRenderDevice());Expect({"prepare","warning"});
    Reset();strict=true;haveRequest=false;assert(!VK_InitRenderDevice());Expect({"prepare","warning","warning"});
    Reset();strict=true;strictRequest.parms.multiSamples=4;assert(!VK_InitRenderDevice());Expect({"prepare","warning","warning"});
    Reset();strict=true;strictRequest.parms.stereo=true;assert(!VK_InitRenderDevice());Expect({"prepare","warning","warning"});
    ...
    Expect({"prepare","window-create","device-init","strict-window-apply","refresh","recreate","resources","refresh","config","ready","input"});
    Reset();strict=true;applyOkay=false;
    assert(!VK_InitRenderDevice() && !deviceAlive && !windowAlive && !glConfig.isInitialized);
    Expect({"prepare","window-create","device-init","strict-window-apply","warning","device-shutdown","window-destroy","warning"});
    Reset();strict=true;services.ApplyScreenParmsStrict=nullptr;
    assert(!VK_InitRenderDevice() && !deviceAlive && !windowAlive && !glConfig.isInitialized);
    Expect({"prepare","window-create","device-init","warning","device-shutdown","window-destroy","warning"});
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix='vid-restart-', dir=ROOT / '.tmp') as temp:
        ...
        init_source.write_text(DEVICE_INIT + function_body(backend_source,
            'static bool VK_ApplyRequestedScreenParms( const renderWindowParms_t& parms )') + '\n'.join(function_body(backend_source, name) for name in (
            'static bool VK_PrepareRenderDevice( void )', 'void VK_ShutdownRenderDevice( void ) {',
            'bool VK_PrepareStartupDevice( char *outReason, int reasonLength )', 'bool VK_InitRenderDevice( void )')) + DEVICE_INIT_MAIN, encoding='utf-8')
