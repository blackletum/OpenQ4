...
def build_evidence(args: argparse.Namespace, *, run_id: str, bridges: tuple[str, ...], reports: list[ReportData]) -> EvidenceData:
    ...
    return EvidenceData(
        ...
        package_layout_contract="adjacent package root: openQ4.app, baseoq4/, and loose runtime files stay together",
        ...
        app_only_move_behavior="completed manual checklist validated; unsupported unless the evidence records that app-only launch now works",
...
def format_checklist(data: EvidenceData) -> str:
    return "\n".join(
        (
            ...
            "- [x] Copied-package launch was checked after moving the whole package payload to a user-writable location.",
            "- [x] Moving only `openQ4.app` was checked and either worked or failed with a clear adjacent-runtime error.",
            ...
            "- [x] Package layout assumptions were checked, including `openQ4.app` beside `baseoq4/` and loose runtime files.",
