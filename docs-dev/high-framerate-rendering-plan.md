...
Status: `In progress` as of `2026-04-16`.
...
Follow-up still needed inside Phase 3:

- Validate the new view interpolation subjectively in SP and MP gameplay at `com_maxfps 240`.
- Subjectively tune the new local-view `pm_presentViewBias` tradeoff in SP and MP gameplay at `com_maxfps 240`, especially around abrupt camera kicks, teleports, and other large one-tic view deltas.
- Continue the effect-side audit beyond the new generic client-entity plus lightning-gun / guide-weapon / gauntlet / hover-effect / Repair Bot / Makron sweep / vehicle-guide / hoverpad / vehicle-crash / projectile-trail / player-powerup / first-person muzzle-tracer / `func_fx` look-at-target coverage and the new render-side BSE service-time split, especially any remaining bespoke visual owners or effect timestamp consumers that still sit outside the current `rvClientEntity`, vehicle-part, entity non-model, player, projectile, or BSE presentation refresh paths.
- Continue the light-side audit beyond the current actor/light/hover/vehicle/exploding-barrel/weapon/MP special-carrier coverage, especially any remaining bespoke render-light paths outside the main entity and vehicle presentation hooks that still update only on the simulation tic.
- Validate cinematic camera interpolation on real cutscenes, especially cut-heavy or looping camera defs, to confirm the presentation snapshot path snaps cleanly at cuts and stays stable through stop/hand-off transitions.
