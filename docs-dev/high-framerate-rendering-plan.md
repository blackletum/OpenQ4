...
- Tightened the SP/MP repeated-state presentation sweep again after a Phase 4 regression hunt: `rvWeapon::Think()` now leaves animation/GUI/light bookkeeping on authoritative simulation frames, the repeated-state view-weapon path refreshes the first-person model through a lightweight render-def update instead of full `UpdateVisuals()`, and `idLight` now only opts into repeated non-model presentation refresh when it actually has a presentation transform delta.
- Revalidated the staged SP `airdefense1` launch-wait harness with `r_swapInterval 0`, `com_maxfps 240`, `com_skipLoadingContinue 1`, and `g_autoSkipCinematics 1`; the earlier degraded sample at `present=13.08 ms (76.5 Hz)` improved to `present=11.10 ms (90.1 Hz)` after the weapon/light presentation pruning, confirming Phase 4 no longer burns as much redraw time on static light defs and duplicate first-person presentation work during post-load gameplay.
- Tightened the repeated redraw path once more after the lingering view-weapon regression/perf follow-up: SP/MP draw-time view-weapon presentation now applies the interpolated pose only for render refresh work, then restores the authoritative weapon physics pose instead of leaving render-only state written back into shared weapon state, while repeated animated-entity render-def refreshes now reuse the current tick's already-built joint frame whenever the renderer has one available. In the staged `airdefense1` autoscreenshot harness, the degraded sample at `present=14.59 ms (68.5 Hz)` recovered to `present=14.04 ms (71.2 Hz)`, returning that scene to the earlier pre-regression neighborhood while removing the latest shared-state contamination path from the first-person weapon draw flow.
