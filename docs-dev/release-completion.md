...
- [x] High-refresh Phase 4 redraw overhead is reduced again: SP/MP first-person repeated-state presentation now avoids full view-weapon `UpdateVisuals()` work on redraw-only frames, and static `idLight` instances no longer re-submit non-model presentation updates every repeated-state frame unless their interpolated transform actually changed. The staged `airdefense1` post-load harness improved from `present=13.08 ms (76.5 Hz)` to `present=11.10 ms (90.1 Hz)` after these pruning fixes.
- [x] High-refresh repeated redraw state handling tightened again: SP/MP draw-time first-person weapon presentation now restores the authoritative weapon pose after render refresh work instead of leaving render-only interpolated state written back into shared weapon state, and repeated animated-entity render updates now skip redundant MD5 callback work once the current tick's skeleton has already been built. The staged `airdefense1` autoscreenshot harness recovered from `present=14.59 ms (68.5 Hz)` to `present=14.04 ms (71.2 Hz)`, bringing that regression back to the earlier pre-break neighborhood while removing the latest shared-state contamination path from the view-weapon draw flow.
