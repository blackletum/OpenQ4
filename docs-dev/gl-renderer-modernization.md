...
The long-term promotion checklist now treats `tools/tests/renderer_promotion_evidence.py` output as the review bundle for that token. A complete token should only be set after `promotion_evidence_summary.md` reports every gate as `pass`, includes Windows x64, Linux x64, and macOS GL 4.1 evidence, and records a final promotion decision. Pending visual references, API captures, platform runs, candidate selection, or performance review keep the summary blocked and keep ARB2 as the default visible path. Approved visual-reference evidence is tracked through `tools/tests/renderer_visual_reference_summary.py`: binary reference screenshots stay external/manual under `.tmp/renderer-references/...`, while the manifest and summary record the reviewed scene matrix, RMS/max thresholds, and human-review checklist. ARB2-or-better performance evidence is tracked through `tools/tests/renderer_performance_summary.py`, which compares the candidate path against explicit `r_renderer arb2` across the agreed gameplay scenes before `perf=arb2-or-better` can be reviewed. Candidate selection is tracked through `tools/tests/renderer_candidate_summary.py`, which records the measured path, acceptance thresholds, required promotion gates, and whether a modern path is currently disqualified or selectable. Cross-platform readiness is tracked through `tools/tests/renderer_platform_summary.py`, which records the per-platform report set and reviewed hardware/driver/GL capability metadata instead of inferring Linux or macOS status from Windows evidence. RenderDoc/API evidence is tracked through `tools/tests/renderer_capture_summary.py`, with RenderDoc `.rdc` files for forced modern/core-profile tiers and API/driver traces for ARB2 compatibility or bind-count claims.
...
Static vertex/index residency remains tracked-only by default. For targeted validation or low-memory experiments, `r_vertexBufferBudget 1` treats `r_vertexBufferMegs` as an LRU budget for static vertex-cache VBO/IBO blocks, while `r_vertexBufferBudgetFrames` protects recently touched blocks from purge. `gfxInfo` prints `Renderer vertex cache:` with fixed, purgable, protected, purged, and over-budget residency fields, and `rendererVertexCacheBudgetSelfTest` verifies the purge/protection contract.
...
- lens-flare accumulation and lens-flare composite variants that reflect the runtime `_lensFlareAccum` contract with scene-color, scene-depth, and accumulation sampler bindings
...
Each linked program records its pass category, material class, compact `rendererPermutationKey_t`, reflected uniform locations, UBOs, SSBOs, samplers, images, attributes, and validated GLSL version. Lens-flare accumulation reflects `uMainTexture` plus `uSceneDepth`, while lens-flare composite reflects `uMainTexture` plus `uLensFlareAccum`, so shader-library self-tests now catch sampler-layout drift before future backend promotion. Shader compile/link diagnostics include the program name, pass, material class, GLSL tier, and permutation key. `gfxInfo` reports program/kind/permutation counts, per-version coverage, reload count, reflection totals, texture-program coverage, and readiness for every shader kind under `Modern GL shader library`, including `lensFlare=accum/composite` plus per-GLSL-tier lens-flare program and sampler coverage. Dedicated `shader-lensflare-gl33`, `shader-lensflare-gl41`, `shader-lensflare-gl43`, `shader-lensflare-gl45`, and `shader-lensflare-gl46` validation cases force the tier ladder and require both lens-flare shader variants to compile and reflect for every GLSL version supported by that context.
...
`tools/tests/renderer_shader_inventory.py` is the static source-inventory companion to the runtime self-test. It verifies the shader-kind enum, descriptor/name contract, GLSL 330/410/430/450 generation, program-slot capacity, draw-record bounds guards, forced shader-tier validation cases, and reload opt-in gate, then writes Markdown/JSON under `.tmp/renderer-validation/<run>/`. The current long-term decision is that these generated inventory reports are validation artifacts only; runtime shader sources remain internal C++ string builders and are not staged as loose shader-content files.
...
The runner covers startup, `gfxInfo`, forced `r_glTier` probes, debug-context fallback, presentation cvar probes, compatibility-gate diagnostics, default-promotion diagnostics, benchmark diagnostics, performance-regression thresholds, clustered-light diagnostics, deferred-resolve diagnostics, forward+ diagnostics, modern-visible diagnostics, modern compatibility diagnostics, GPU-driven diagnostics, lens-flare shader-tier coverage, dedicated-server disabled-BSE startup coverage, lens-flare cross-platform sign-off reporting, and all renderer foundation self-tests without entering gameplay. The opt-in gameplay runner owns map-loading evidence; `renderer_gameplay_benchmark.py --profile lensflare-signoff` is the cross-platform lens-flare visual/pacing profile for Windows, Linux, and macOS release claims.
