...
- `r_shadowMapReport` and `gfxInfo` report the modern shadow plan: mapped, stencil-fallback, and skipped lights; projected/point/CSM counts; caster/receiver totals; budgeted shadow size, pixels, and update cadence; and unsupported-resource fallback reasons. Deferred resolve and forward+ shaders consume the clustered shadow descriptor policy from that plan, and visible-frame handoff rejects unresolved shadow/stencil fallbacks instead of double-shadowing or silently dropping shadows.
...
- `ModernShadowPlanner` builds the shared per-frame shadow policy from packet-scene lights, applies the current shadow-map contract and renderer budget, records mapped/stencil-fallback/skipped decisions with caster/receiver cost data, and feeds descriptor indices and fallback policy into clustered light records for deferred and forward+ receiver work.
