...
| Performance Preset | `choiceDef` | `set_sys_perf_preset_val` | `com_performancePreset` via `applyPerformancePreset` | `minimum`, `lowpower`, `performance`, `balanced`, `quality`, `ultra` | Applies a coordinated profile for machine spec, resolution scale, AA, frame cap, texture pressure, renderer budgets, light grids, and audio cost. The session-menu bridge forwards preset command arguments through engine validation, then refreshes video, quality, and aspect widgets. Keeps optional shadow maps and post effects off as separate opt-in settings. |
