...
- **openQ4-game source-input contract:** openQ4 stages regular source files from the companion repo into `.tmp/gamelibs_stage/`, writes the `openq4_gamelibs_stage_manifest.json` hash/git-state manifest, requires it during Meson configure, and validates the staging script. The companion repo documents and CI-checks its Linux role as source input rather than a standalone Linux GameLibs target.
...
- Add longer runtime coverage that enters maps/gameplay under Linux Wayland, including suspend/resume, repeated fullscreen/resize stress, controller hotplug, and audio-device churn.
...
- Decide later whether `openQ4-game` should grow a standalone Linux GameLibs build. For now the explicit project decision is that Linux modules are compiled through openQ4's staged engine build.
