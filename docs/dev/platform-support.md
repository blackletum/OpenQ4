...
- As of March 30, 2026, Linux and experimental macOS default to the SDL3 backend and keep `-Dplatform_backend=native` as a fallback/comparison path.
...
- The native Linux X11/GLX and experimental macOS Cocoa/OpenGL backends remain available through `-Dplatform_backend=native` for comparison and rollback while SDL3 remains the release path. On SteamOS, the SDL3 path also watches application lifecycle events so suspend/resume and foreground/background transitions flow through the same input release, rumble stop, config write, and controller reacquire behavior used by the normal event pump.
