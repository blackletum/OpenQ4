...
static void R_ModernGLShaderLibrary_BuildVertexSource( int glslVersion, modernGLShaderProgramKind_t kind, char *buffer, int bufferSize ) {
	...
	idStr::snPrintf(
		...
		"    mat3 localToView = mat3(uModelViewMatrix);\n"
		...
		"    vViewPosition = (uModelViewMatrix * vec4(attr_Position, 1.0)).xyz;\n"
...
static void R_ModernGLShaderLibrary_BuildFragmentSource( int glslVersion, modernGLShaderProgramKind_t kind, char *buffer, int bufferSize ) {
	...
	const char *clusterHeader =
		...
		"}\n"
		"vec3 ModernClusterEvaluateLight(ModernClusterLightRecord light, vec3 viewPosition, vec3 normal, float specular, out float attenuation) {\n"
		...
		"    float spec = pow(clamp(dot(normal, halfDir), 0.0, 1.0), 24.0) * specular;\n"
	...
	if ( kind == MODERN_GL_SHADER_DEFERRED_LIGHT_RESOLVE ) {
		idStr::snPrintf(
			...
			"    int indexBase = int(max(uClusterGrid.viewport.z, 0.0));\n"
			...
			"        uvec4 lightIndices = ModernClusterFetchIndex(indexBase + clusterIndex * indexGroups + (i >> 2));\n"
			...
			"        vec3 contribution = ModernClusterEvaluateLight(light, viewPosition, normal, material.g, attenuation);\n"
	...
	if ( kind == MODERN_GL_SHADER_CLUSTERED_FORWARD_OPAQUE || kind == MODERN_GL_SHADER_CLUSTERED_FORWARD_ALPHA_TEST ) {
		idStr::snPrintf(
			...
			"        float shadowVisibility = ModernClusterShadowVisibility(light);\n"
			"        float attenuation = 0.0;\n"
			"        vec3 contribution = ModernClusterEvaluateLight(light, vViewPosition, materialNormal, specular, attenuation);\n"
