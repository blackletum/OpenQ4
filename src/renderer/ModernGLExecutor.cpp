...
static bool R_ModernGLExecutor_IsForwardPlusDecalCommand( const modernGLSubmitCommand_t &command ) {
	...
	return materialRecord->sortGroup == MATERIAL_RESOURCE_SORT_DECAL
		|| ( draw != NULL && draw->legacyDrawSurf != NULL && draw->legacyDrawSurf->decalColorCache != NULL );
}
...
static void R_ModernGLExecutor_LocalParamsForCommand( const modernGLSubmitCommand_t &command, float params[4] ) {
	...
	case MODERN_GL_SHADER_TRANSPARENT_FORWARD:
	case MODERN_GL_SHADER_FOG_BLEND:
		if ( command.shaderKind == MODERN_GL_SHADER_TRANSPARENT_FORWARD && R_ModernGLExecutor_IsForwardPlusDecalCommand( command ) ) {
			params[3] = 1.0f;
			break;
		}
		break;
...
static GLenum R_ModernGLExecutor_TextureTargetForGroup( const GLenum *targets, GLsizei index ) {
	return targets != NULL && targets[index] != 0 ? targets[index] : GL_TEXTURE_2D;
}

static void R_ModernGLExecutor_BindTextureGroup( GLuint first, GLsizei count, const GLuint *textures, const GLenum *targets, GLuint sampler, bool bindSampler, modernGLExecutorStats_t &stats ) {
	...
	if ( stats.tierUsesMultiBind && glBindTextures != NULL ) {
		if ( R_GLStateCache().BindTextures( first, clampedCount, textures, targets ) ) {
			stats.lowOverheadTextureMultiBindBatches++;
		}
		if ( bindSampler && glBindSamplers != NULL ) {
			GLuint samplers[MATERIAL_RESOURCE_TABLE_TEXTURE_ARRAY_CAPACITY];
			for ( GLsizei i = 0; i < clampedCount; ++i ) {
				samplers[i] = sampler;
			}
			if ( R_GLStateCache().BindSamplers( first, clampedCount, samplers ) ) {
	...
	for ( GLsizei i = 0; i < clampedCount; ++i ) {
		const GLenum target = R_ModernGLExecutor_TextureTargetForGroup( targets, i );
		R_GLStateCache().ActiveTextureUnit( static_cast<int>( first + i ) );
		if ( R_GLStateCache().BindTexture( static_cast<int>( first + i ), target, textures[i] ) ) {
			stats.lowOverheadClassicTextureBinds++;
		}
		if ( bindSampler && glBindSampler != NULL ) {
			R_GLStateCache().BindSampler( static_cast<int>( first + i ), sampler );
		}
	}
}

static void R_ModernGLExecutor_BindTextureGroup( GLuint first, GLsizei count, const GLuint *textures, modernGLExecutorStats_t &stats ) {
	R_ModernGLExecutor_BindTextureGroup( first, count, textures, NULL, rg_modernGLExecutorLowOverheadSampler, rg_modernGLExecutorLowOverheadSampler != 0, stats );
}

static GLenum R_ModernGLExecutor_ShadowTextureTarget( const rendererShadowTextureBinding_t &binding, const unsigned int fallbackTarget ) {
	return binding.target != 0 ? static_cast<GLenum>( binding.target ) : static_cast<GLenum>( fallbackTarget );
}

static GLuint R_ModernGLExecutor_ShadowTextureName( const rendererShadowTextureBinding_t &binding ) {
	return binding.ready ? binding.texture : 0;
}

static void R_ModernGLExecutor_ApplyShadowTextureParameters( GLuint first, GLsizei count, const GLuint *textures, const GLenum *targets ) {
	for ( GLsizei i = 0; i < count; ++i ) {
		const GLenum target = R_ModernGLExecutor_TextureTargetForGroup( targets, i );
		if ( textures[i] != 0 && ( target == GL_TEXTURE_2D || target == GL_TEXTURE_CUBE_MAP ) ) {
			R_GLStateCache().ActiveTextureUnit( static_cast<int>( first + i ) );
...
static bool R_ModernGLExecutor_BindModernShadowTextures( GLuint program, modernGLExecutorStats_t &stats ) {
	...
	GLuint shadowTextures[MODERN_GL_SHADOW_TEXTURE_UNIT_COUNT];
	GLenum shadowTargets[MODERN_GL_SHADOW_TEXTURE_UNIT_COUNT];
	shadowTextures[0] = R_ModernGLExecutor_ShadowTextureName( bindings.projectedAtlas );
	shadowTargets[0] = R_ModernGLExecutor_ShadowTextureTarget( bindings.projectedAtlas, GL_TEXTURE_2D );
	shadowTextures[1] = R_ModernGLExecutor_ShadowTextureName( bindings.pointAtlas );
	shadowTargets[1] = R_ModernGLExecutor_ShadowTextureTarget( bindings.pointAtlas, GL_TEXTURE_CUBE_MAP );
	for ( int i = 0; i < RENDERER_SHADOW_TEXTURE_MOMENT_COUNT; ++i ) {
		const int projectedSlot = 2 + i;
		const int pointSlot = 2 + RENDERER_SHADOW_TEXTURE_MOMENT_COUNT + i;
		shadowTextures[projectedSlot] = R_ModernGLExecutor_ShadowTextureName( bindings.projectedMoments[i] );
		shadowTargets[projectedSlot] = R_ModernGLExecutor_ShadowTextureTarget( bindings.projectedMoments[i], GL_TEXTURE_2D );
		shadowTextures[pointSlot] = R_ModernGLExecutor_ShadowTextureName( bindings.pointMoments[i] );
		shadowTargets[pointSlot] = R_ModernGLExecutor_ShadowTextureTarget( bindings.pointMoments[i], GL_TEXTURE_CUBE_MAP );
	}
	R_ModernGLExecutor_BindTextureGroup( MODERN_GL_SHADOW_TEXTURE_UNIT_PROJECTED_ATLAS, MODERN_GL_SHADOW_TEXTURE_UNIT_COUNT, shadowTextures, shadowTargets, 0, true, stats );
	R_ModernGLExecutor_ApplyShadowTextureParameters( MODERN_GL_SHADOW_TEXTURE_UNIT_PROJECTED_ATLAS, MODERN_GL_SHADOW_TEXTURE_UNIT_COUNT, shadowTextures, shadowTargets );
...
static bool R_ModernGLExecutor_ForwardPlusCommandRequested( const modernGLExecutorStats_t &stats, const modernGLSubmitCommand_t &command ) {
	...
	if ( stats.forwardPlusDecalOnlyRequested && !R_ModernGLExecutor_IsForwardPlusDecalCommand( command ) ) {
		return false;
	}
	return true;
}
...
static bool R_ModernGLExecutor_ForwardPlusMaterialSupported( const modernGLSubmitCommand_t &command, modernGLExecutorStats_t &stats ) {
	...
	const bool decalMaterial = R_ModernGLExecutor_IsForwardPlusDecalCommand( command );
	if ( materialRecord->fallbackReason != MATERIAL_RESOURCE_FALLBACK_NONE
...
static void R_ModernGLExecutor_SubmitForwardPlus( modernGLExecutorStats_t &stats ) {
	...
	R_RendererMetrics_BeginGpuTimer( RENDERER_GPU_TIMER_MODERN_FORWARD );
	{
		...
		for ( int i = 0; i < rg_modernGLSubmitPlan.NumCommands(); ++i ) {
			...
			const bool decalCommand = R_ModernGLExecutor_IsForwardPlusDecalCommand( command );
			if ( !R_ModernGLExecutor_CommandVisibleForModernPath( command, &stats, false ) ) {
				continue;
			}
			...
			if ( transparent ) {
				const materialResourceTableRecord_t *transparentMaterialRecord = R_MaterialResourceTable_RecordForIndex( command.materialTableIndex );
				const bool decalMaterial = transparentMaterialRecord != NULL && R_ModernGLExecutor_IsForwardPlusDecalCommand( command );
				R_GLStateCache().SetBlendEnabled( true );
			...
			const materialResourceTableRecord_t *commandMaterialRecord = R_MaterialResourceTable_RecordForIndex( command.materialTableIndex );
			const bool applyPolygonOffset = transparent
				&& commandMaterialRecord != NULL
				&& R_ModernGLExecutor_IsForwardPlusDecalCommand( command )
...
void R_ModernGLExecutor_PrepareFrame( const idScenePacketFrame &packetFrame, const idRenderGraph &graph ) {
	...
	if ( r_rendererMetrics.GetInteger() >= 2 && enabled ) {
		...
		common->Printf(
			"modernGpuDrivenMDI drawRecords=%d ready=%d/%d bucketReady=%d bytes=%d/%d bucketBytes=%d buckets=%d bucketed=%d compacted=%d generated=%d hiz=%d/%d rejected=%d invalidBucket=%d overflow=%d indirectBytes=%d multiDrawCalls=%d multiDrawBatches=%d fallback=%d streamed=%d upload=%d/%d validationDeferred=%d skipped=%d\n",
...
void R_ModernGLExecutor_PrintGfxInfo( void ) {
	...
	common->Printf(
		...
		rg_modernGLExecutorStats.submitPlanSortVertexBufferBatchSavings );
	common->Printf(
		"Modern GL GPU-driven MDI: drawRecords=%d ready=%d/%d bucketReady=%d bytes=%d/%d bucketBytes=%d buckets=%d bucketed=%d compacted=%d generated=%d hiz=%d/%d rejected=%d invalidBucket=%d overflow=%d indirectBytes=%d multiDrawCalls=%d multiDrawBatches=%d fallback=%d streamed=%d upload=%d/%d validationDeferred=%d skipped=%d\n",
		rg_modernGLExecutorStats.gpuDrivenDrawRecords,
		rg_modernGLExecutorStats.drawRecordBufferReady ? 1 : 0,
		rg_modernGLExecutorStats.drawRecordIndexBufferReady ? 1 : 0,
		rg_modernGLExecutorStats.gpuDrivenBucketBufferReady ? 1 : 0,
...
static bool R_ModernGLExecutor_RunGpuDrivenInvalidIndexSelfTest( void ) {
	if ( rg_modernGLExecutorComputeProgram == 0 || rg_modernGLExecutorSceneSSBO == 0 || rg_modernGLExecutorIndirectBuffer == 0 || rg_modernGLExecutorBucketSSBO == 0 || rg_modernGLExecutorValidationSSBO == 0 ) {
		return false;
	}
	if ( glDispatchCompute == NULL || glMemoryBarrier == NULL || glUniform1ui == NULL || glUniform4f == NULL ) {
		return false;
	}
	...
	for ( int i = 0; i < 2; ++i ) {
		...
		sceneRecords[i].indirect[1] = 0u;
		sceneRecords[i].indirect[2] = 0u;
		sceneRecords[i].indirect[3] = static_cast<GLuint>( i );
	}
	sceneRecords[0].ids[0] = 64u;
	sceneRecords[1].ids[0] = 0u;
	bucketRecords[0].header[0] = 0u;
	bucketRecords[0].header[1] = 0u;
	bucketRecords[0].header[2] = 0u;
	bucketRecords[0].header[3] = 0u;
	...
	R_ModernGLExecutor_UnbindGpuDrivenBuffers();
	if ( !readbackOk ) {
		common->Printf( "RendererGpuDriven self-test failed: invalid-index counter readback unavailable\n" );
		return false;
	}
	if ( gpuCounters[MODERN_GL_GPU_COUNTER_PROCESSED] != 2u
		...
		common->Printf(
			"RendererGpuDriven self-test failed: invalid-index counters mismatch (processed=%u eligible=%u generated=%u invalidBucket=%u overflow=%u)\n",
...
bool RendererGpuDriven_RunSelfTest( void ) {
	...
	if ( !R_ModernGLExecutor_RunGpuDrivenInvalidIndexSelfTest() ) {
		return false;
	}

	common->Printf(
		"RendererGpuDriven self-test passed (resources=%d compute=%d source=%d eligible=%d generated=%d culled=%d visible=%d cpu=%d/%d/%d gpu=%d/%d/%d clusters=%d/%d mismatches=%d readbacks=%d drawRecords=%d buckets=%d bucketed=%d compacted=%d invalidProbe=1 hiz=%d/%d/%d indirect=%d multiDraw=%d indirectCalls=%d dispatches=%d)\n",
...
typedef struct modernGLVisiblePathSelfTestFrame_s {
	drawSurf_t drawSurfs[2];
	drawSurf_t *drawSurfPtrs[2];
	srfTriangles_t geometry;
	vertCache_t ambientCache;
	vertCache_t indexCache;
	viewEntity_t viewEntity;
	viewDef_t worldView;
} modernGLVisiblePathSelfTestFrame_t;

static bool R_ModernGLExecutor_BuildVisiblePathSelfTestFrame( idScenePacketFrame &packetFrame, idRenderGraph &graph, modernGLVisiblePathSelfTestFrame_t &frameData ) {
	packetFrame.Clear();
	graph.Clear();
	memset( &frameData, 0, sizeof( frameData ) );

	if ( !R_ModernGLExecutor_InitSelfTestTriangleGeometry( frameData.geometry, frameData.ambientCache, frameData.indexCache, "RendererVisiblePath" ) ) {
		return false;
	}
	for ( int i = 0; i < 2; ++i ) {
		frameData.drawSurfs[i].geo = &frameData.geometry;
		if ( tr.defaultMaterial != NULL ) {
			frameData.drawSurfs[i].material = tr.defaultMaterial;
			frameData.drawSurfs[i].sort = tr.defaultMaterial->GetSort();
		}
	}

	frameData.drawSurfPtrs[0] = &frameData.drawSurfs[0];
	frameData.drawSurfPtrs[1] = &frameData.drawSurfs[1];
	frameData.drawSurfs[0].space = &frameData.viewEntity;
	frameData.drawSurfs[1].space = &frameData.viewEntity;
	frameData.worldView.viewEntitys = &frameData.viewEntity;
	frameData.worldView.drawSurfs = frameData.drawSurfPtrs;
	frameData.worldView.numDrawSurfs = 2;

	if ( !packetFrame.AddScene( &frameData.worldView, true ) ) {
		return false;
	}
	...
	for ( int i = 0; i < 2; ++i ) {
		if ( !packetFrame.AddDrawPacket( &frameData.drawSurfs[i], RENDER_PASS_DEPTH, i ) ) {
			return false;
		}
	}
	...
	for ( int i = 0; i < 2; ++i ) {
		if ( !packetFrame.AddDrawPacket( &frameData.drawSurfs[i], RENDER_PASS_SHADOW_MAP, i ) ) {
			return false;
		}
	}
...
bool RendererVisiblePath_RunSelfTest( void ) {
	...
	idScenePacketFrame packetFrame;
	idRenderGraph graph;
	modernGLVisiblePathSelfTestFrame_t frameData;
	if ( !R_ModernGLExecutor_BuildVisiblePathSelfTestFrame( packetFrame, graph, frameData ) ) {
...
typedef struct modernGLGBufferSelfTestFrame_s {
	drawSurf_t drawSurfs[2];
	drawSurf_t *drawSurfPtrs[2];
	srfTriangles_t geometry;
	vertCache_t ambientCache;
	...
	viewEntity_t viewEntity;
	viewDef_t worldView;
} modernGLGBufferSelfTestFrame_t;

static bool R_ModernGLExecutor_BuildGBufferSelfTestFrame( idScenePacketFrame &packetFrame, idRenderGraph &graph, modernGLGBufferSelfTestFrame_t &frameData ) {
	packetFrame.Clear();
	graph.Clear();
	memset( &frameData, 0, sizeof( frameData ) );
	if ( !R_ModernGLExecutor_InitSelfTestTriangleGeometry( frameData.geometry, frameData.ambientCache, frameData.indexCache, "RendererGBuffer" ) ) {
		return false;
	}
	for ( int i = 0; i < 2; ++i ) {
		frameData.drawSurfs[i].geo = &frameData.geometry;
	...
	for ( int i = 0; i < 2; ++i ) {
		if ( !packetFrame.AddDrawPacket( &frameData.drawSurfs[i], RENDER_PASS_AMBIENT, i ) ) {
			return false;
		}
	}
	packetFrame.FinishScene();
...
bool RendererGBuffer_RunSelfTest( void ) {
	...
	modernGLGBufferSelfTestFrame_t frameData;
	idScenePacketFrame packetFrame;
	idRenderGraph graph;
	if ( !R_ModernGLExecutor_BuildGBufferSelfTestFrame( packetFrame, graph, frameData ) ) {
