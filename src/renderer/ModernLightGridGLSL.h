...
// The existing .lightgrid/.lightgridpack atlas layout, sampled in the surface
// shader so normal mapping, alpha coverage and MSAA share the direct-light draw.
#ifndef __MODERN_LIGHT_GRID_GLSL_H__
...
vec4 ModernBakedIrradiance(vec3 position, vec3 normal, bool pbr) {
    ...
    vec3 worldPosition = uBakedGrid[6].xyz + ModernBakedToWorld(position);
    vec3 worldNormal = normalize(ModernBakedToWorld(normal));
